...
| Disallowed top-level nav/redirects/popups blocked before commit; denied pages removed; cross-origin subresources render; frames need grant                                    | Automated: `test/origin-scope-host.contract.test.ts`, `test/browser-origin-scope.integration.test.ts`.                                                                                     |
...
| CLI has no owner authority; grant administration fails closed to authenticated Settings; URL open is agent-attributed                                                         | Automated: `test/public-plugin.contract.test.tsx` (privilege escalation, Activity attribution, expired file-transfer elevation).                                                           |
