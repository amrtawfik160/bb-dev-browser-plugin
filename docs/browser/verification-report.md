...
| Disallowed web/non-web top-level nav/redirects/popups blocked before commit; denied pages removed; exact `about:blank` only internal exception; cross-origin subresources render; frames need grant     | Automated: `test/origin-scope-host.contract.test.ts`, `test/browser-origin-scope.integration.test.ts`.                                                                                                                           |
